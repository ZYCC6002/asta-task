...
def semantic_search(query: str, top_k: int = 5):
    ...
    while True:
        ...
        if os.path.exists(np_path):
            index_embeddings = mx.load(np_path)
    ...
    with open("data/arxiv_concatenated.jsonl", "r", encoding="utf-8") as file:
        for score, table_id in top_candidates:
            print(f"Score: {score:.4f}, Table ID: {table_id}")
...
if __name__ == "__main__":
    semantic_search(f"""Papers involving atoms and nucleus""", top_k=5)
    
    # import mlx.core as mx

    # embeddings = mx.load("data/doc_embeddings/emb_b1.npy")
    # print("embeddings shape:", embeddings.shape)
    # print("Loaded embeddings:", embeddings[:5][:])